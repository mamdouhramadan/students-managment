...
const FamilyMembersLogic = () => {
    ...
    const handlelInputChange = (e) => {
        setFamilyMember({ ...newFamilyMember, [e.target.name]: e.target.value })

        if (e.target.name === 'nationality') {
            const nationality = studentStore.nationalities.find(obj => obj.ID === Number(e.target.value));
            studentActions.updateNewFamilyMember({ ...newFamilyMember, [e.target.name]: nationality })
        } else {
            studentActions.updateNewFamilyMember({ ...newFamilyMember, [e.target.name]: e.target.value })
        }
    }
